    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="auth-header">
          <h1 className="auth-title">租户管理登录</h1>
          <p className="auth-subtitle">品牌/店铺管理后台，请使用租户账号登录</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="form-group">
            ...
            <input
              ...
              placeholder="请输入邮箱" disabled={isLoading}
              autoComplete="email"
            />
          ...
          <button type="submit" className="auth-button primary" disabled={isLoading}>
            {isLoading ? <><span className="spinner"></span>登录中...</> : '租户登录'}
          </button>
        </form>

        <div className="auth-footer">
          <Link to="/auth/admin/login" className="auth-link">超管登录 ←</Link>
        </div>
      </div>
    </div>
  );
